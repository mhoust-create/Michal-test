...
// Slide-up editable sheet for a specific day
function DayEditSheet({ dateStr, supplements, log, onToggle, onClose }) {
  ...
  return (
    <>
      ...
      >
        ...
        {/* Supplement list with checkboxes */}
                    ...
                    <button className="flex-1 min-w-0 text-left" onClick={() => setDetailSupp(supp)}>
                      <div className="flex items-center gap-1.5">
                        <span
                          ...
                          style={{ color: allTaken ? '#6b7280' : '#fff', textDecoration: allTaken ? 'line-through' : 'none' }}
                        >
                          {supp.name}
                        </span>
                      ...
                      {Array.from({ length: supp.timesPerDay }).map((_, i) => {
                        ...
                        return (
                          <div key={i} className="flex flex-col items-center gap-1">
                            {lbl && (
                              <span style={{ fontSize: '0.48rem', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{lbl}</span>
                            )}
                            <button
...
export function CalendarView({ supplements, log, today, onToggle }) {
  ...
  const [editDateStr, setEditDateStr] = useState(null);
  ...
  return (
    <div className="p-4 pb-6">
          ...
          return (
            ...
            >
              ...
              {!isSelected && status !== 'none' && (
                <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full" style={{ background: status === 'full' ? '#22c55e' : '#f59e0b' }} />
              )}
            </button>
          );
        })}
      ...
      {/* Selected day overview (read-only) — tap to edit */}
      {selectedDateStr && (
        <button
          className="w-full rounded-2xl p-4 text-left slide-up"
          style={{ background: '#161b22', border: '1px solid #21262d' }}
          onClick={() => setEditDateStr(selectedDateStr)}
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-gray-500">Tap to edit ✏️</span>
          </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex gap-1">
                        {Array.from({ length: total }).map((_, i) => (
                          <span key={i} className="w-2 h-2 rounded-full" style={{ background: sl[i] ? supp.color : '#21262d' }} />
                        ))}
                      </div>
                      <span
