...
export function CalendarView({ supplements, log, today }) {
  const [viewDate, setViewDate] = useState(() => { const d = new Date(today); return new Date(d.getFullYear(), d.getMonth(), 1); });
  ...
  const prevMonth = () => { setSelectedDay(null); setViewDate(d => new Date(d.getFullYear(), d.getMonth() - 1, 1)); };
  const nextMonth = () => { setSelectedDay(null); setViewDate(d => new Date(d.getFullYear(), d.getMonth() + 1, 1)); };
  ...
  const monthFull = [], monthPartial = [];
  for (let d = 1; d <= daysInMonth; d++) {
    const st = getDayStatus(getDateStr(year, month, d), supplements, log);
  ...
  return (
    <div className="p-4 pb-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <button onClick={prevMonth} className="w-9 h-9 rounded-xl flex items-center justify-center text-gray-300 text-lg" style={{ background: '#161b22' }}>‹</button>
        <h2 className="text-base font-bold text-white">{monthName}</h2>
        <button onClick={nextMonth} className="w-9 h-9 rounded-xl flex items-center justify-center text-gray-300 text-lg" style={{ background: '#161b22' }}>›</button>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1 mb-5">
        ...
        {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => {
          ...
          return (
            <button key={day} onClick={() => !isFuture && setSelectedDay(day === selectedDay ? null : day)} disabled={isFuture}
              className="relative aspect-square rounded-xl flex items-center justify-center text-sm font-medium transition-all"
              style={{
                background: isSelected ? '#22c55e' : status === 'full' ? '#22c55e22' : status === 'partial' ? '#f59e0b18' : '#161b22',
                color: isSelected ? '#fff' : isFuture ? '#2d3748' : isToday ? '#22c55e' : '#e6edf3',
                border: isToday && !isSelected ? '2px solid #22c55e' : '2px solid transparent',
              }}>
              {day}
              {!isSelected && status !== 'none' && <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full" style={{ background: status === 'full' ? '#22c55e' : '#f59e0b' }} />}
            </button>
          );
        })}
      </div>
      <div className="flex gap-5 mb-5 text-xs text-gray-500">
        <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full" style={{ background: '#22c55e' }} />All taken</div>
        <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full" style={{ background: '#f59e0b' }} />Partial</div>
      </div>
            ...
            {supplements.map(supp => {
              ...
              return (
                <div key={supp.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-2"><span className="text-base">{supp.emoji}</span><span className="text-sm text-gray-300">{supp.name}</span></div>
                  <div className="flex items-center gap-2">
                    <div className="flex gap-1">{Array.from({ length: supp.timesPerDay }).map((_, i) => <span key={i} className="w-2 h-2 rounded-full" style={{ background: sl[i] ? supp.color : '#21262d' }} />)}</div>
                    <span className="text-xs font-semibold tabular-nums" style={{ color: taken === supp.timesPerDay ? '#22c55e' : taken > 0 ? '#f59e0b' : '#4b5563' }}>{taken}/{supp.timesPerDay}</span>
                  </div>
                </div>
              );
            })}
...
function computeStreak(supplements, log, today) {
  ...
  while (true) {
    ...
    if (getDayStatus(ds, supplements, log) !== 'full') break;
    streak++;
